...
import {
  Activity, AlertTriangle, ArrowLeft, CalendarDays, Database, Download, Eye,
  FileDown, FileText, Filter, RefreshCw, Search, ShieldCheck, UserRound, X
} from "lucide-react";
...
export default function Audit() {
  ...
  const systemKey = String(params.get("system") || "").trim().toLowerCase();
  const requestedName = String(params.get("name") || "").trim();
  const [rows, setRows] = useState<AuditRow[]>([]);
  const [source, setSource] = useState<{ key?: string; label?: string; configured?: boolean; error?: string | null } | null>(null);
  ...
  const systemName = source?.label || requestedName || systemKey.toUpperCase() || "Sistema";

  const load = async () => {
    if (!systemKey) {
      setRows([]);
      setSource(null);
      setError("Abra a auditoria a partir do Monitoramento de um órgão.");
      setLoading(false);
      return;
    }
    setLoading(true);
    setError("");
    try {
      const qs = new URLSearchParams({ system: systemKey, limit: "4000" });
      ...
      setSource(data.system || null);
      if (!data.ok && data.system?.error) setError(data.system.error);
    } catch (e) {
      setRows([]);
  ...
  useEffect(() => { void load(); }, [systemKey, from, to]);
  ...
  const stats = useMemo(() => ({
    ...
    modules: new Set(filtered.map(r => r.module).filter(Boolean)).size,
  }), [filtered]);

  const exportCsv = () => {
    const headers = ["Data/Hora","Órgão","Usuário","E-mail","Cargo","Módulo","Ação","Criticidade","Processo","Descrição","Antes","Depois","Dispositivo","IP"];
    const data = filtered.map(r => [
      fmtDate(r.created_at),systemName,r.user_name,r.user_email,r.user_role,r.module,
      ACTION_LABELS[r.action_type || ""] || r.action_type,severity(r.action_type),r.process_number,
      r.description,jsonText(r.before_data),jsonText(r.after_data),r.user_agent,r.ip_address
    ]);
    ...
    a.href = url;
    a.download = `auditoria-${systemKey}-${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  ...
  const printReport = () => {
    ...
    if (!w) return;
    const lines = filtered.slice(0,1000).map(r => `<tr><td>${safeHtml(fmtDate(r.created_at))}</td><td>${safeHtml(r.user_name || "—")}</td><td>${safeHtml(r.module || "—")}</td><td>${safeHtml(ACTION_LABELS[r.action_type || ""] || r.action_type || "—")}</td><td>${safeHtml(r.process_number || "—")}</td><td>${safeHtml(r.description || "—")}</td></tr>`).join("");
    w.document.write(`<!doctype html><html><head><title>Relatório de Auditoria</title><style>body{font-family:Arial,sans-serif;color:#172033;padding:32px}h1{font-size:22px;margin:0}p{color:#64748b}table{width:100%;border-collapse:collapse;margin-top:24px;font-size:10px}th,td{border:1px solid #dbe2ea;padding:6px;text-align:left;vertical-align:top}th{background:#071d35;color:white}.meta{display:flex;gap:24px;margin-top:14px;font-size:12px}.logo{font-weight:900;color:#071d35}.small{font-size:10px;color:#64748b;margin-top:20px}@media print{body{padding:0}}</style></head><body><div class="logo">MW TECH Control</div><h1>Relatório de Auditoria</h1><p>${safeHtml(systemName)}</p><div class="meta"><b>Registros: ${filtered.length}</b><span>Período: ${safeHtml(from || "início")} até ${safeHtml(to || "hoje")}</span><span>Gerado em: ${safeHtml(new Date().toLocaleString("pt-BR"))}</span></div><table><thead><tr><th>Data/Hora</th><th>Usuário</th><th>Módulo</th><th>Ação</th><th>Processo</th><th>Descrição</th></tr></thead><tbody>${lines}</tbody></table><div class="small">Relatório administrativo gerado pelo MW TECH Control.</div><script>window.onload=()=>window.print()<\/script></body></html>`);
    w.document.close();
  };
  ...
  return <div className="mx-auto w-full max-w-[1480px] space-y-6">
    ...
    {!systemKey && <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-sm font-semibold text-blue-800">Selecione uma prefeitura no Monitoramento e clique em Auditoria.</div>}
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[
        ...
        ["Usuários envolvidos", stats.users, UserRound],
        ["Módulos auditados", stats.modules, Database],
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><b className="text-sm text-[#07182d]">Linha do tempo de auditoria</b><p className="text-xs text-slate-500">{filtered.length} evento(s) encontrado(s) em {systemName}</p></div><Database size={18} className="text-slate-400"/></div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[980px] text-left text-xs">
          ...
          <tbody>
            {loading ? <tr><td colSpan={7} className="px-4 py-16 text-center text-sm font-semibold text-slate-500">Carregando auditoria...</td></tr> :
            filtered.length === 0 ? <tr><td colSpan={7} className="px-4 py-16 text-center text-sm text-slate-500">Nenhum evento encontrado para este órgão.</td></tr> :
            filtered.slice(0,1000).map((r,i) => {
              const sev=severity(r.action_type);
              return <tr key={r.id || `${systemKey}-${r.created_at}-${i}`} className="border-t border-slate-100 hover:bg-slate-50/70">
