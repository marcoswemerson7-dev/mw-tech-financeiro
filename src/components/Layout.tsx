...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Support notifications must never block the rest of the system.
      }
    };
    void poll();
  ...
  const saveProfile = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const avatarFile = (form.elements.namedItem("avatar") as HTMLInputElement)?.files?.[0] || null;
      await updateUserProfile({ name: values.name || "", cargo: values.cargo || "", avatarFile });
      await refresh();
  ...
  return (
    <div className="min-h-screen bg-[#f3f6fa] text-slate-900">
      ...
      {profileModal && <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/65 p-4">
        <div className="w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-[0_30px_100px_rgba(0,0,0,.35)]">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5"><div><h3 className="text-xl font-black text-[#07182d]">Meu perfil</h3><p className="text-xs text-slate-500">Atualize sua foto, nome e cargo.</p></div><button onClick={() => setProfileModal(false)} className="grid size-9 place-items-center rounded-xl hover:bg-slate-100"><X size={20} /></button></div>
          <form onSubmit={saveProfile} className="space-y-5 p-6">
            <div className="flex items-center gap-4"><div className="relative">{avatarUrl ? <img src={avatarUrl} alt={userName} className="size-20 rounded-2xl border border-slate-200 object-cover" /> : <span className="grid size-20 place-items-center rounded-2xl bg-[#07182d] text-xl font-black text-[#e5b557]">{initials}</span>}<span className="absolute -bottom-2 -right-2 grid size-8 place-items-center rounded-full border-2 border-white bg-[#d8a338] text-[#07182d]"><Camera size={15} /></span></div><div><b className="block text-sm text-[#07182d]">Foto do perfil</b><p className="mt-1 text-xs text-slate-500">PNG ou JPG, até 4 MB.</p></div></div>
            <label className="block text-sm font-bold text-slate-600">Alterar foto<input type="file" name="avatar" accept="image/png,image/jpeg,image/webp" className="mt-2 block w-full rounded-xl border border-slate-200 p-3 text-sm" /></label>
