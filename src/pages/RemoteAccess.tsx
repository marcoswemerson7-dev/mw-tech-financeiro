...
export default function RemoteAccess() {
  ...
  const grouped = useMemo(() => {
    ...
    const filtered = devices.filter((item) => !q || [item.orgao, item.dispositivo, item.setor, item.usuario, item.anydesk_id].some((value) => String(value || "").toLowerCase().includes(q)));
    const map = new Map<string, RemoteDevice[]>();
    filtered.forEach((item) => map.set(item.orgao, [...(map.get(item.orgao) || []), item]));
    ...
    <div className="space-y-5">
      <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_14px_42px_rgba(7,24,45,.06)]">
        <div className="flex flex-col gap-5 bg-[radial-gradient(circle_at_top_right,_rgba(214,163,58,.11),_transparent_35%),linear-gradient(135deg,#fff_0%,#f9fbfd_100%)] p-6 lg:flex-row lg:items-center lg:justify-between xl:p-7">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm font-medium text-slate-500">Centralize os computadores das prefeituras e órgãos e abra o AnyDesk diretamente pelo painel.</p>
          </div>
      ...
      <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm">
        <div className="relative">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar órgão, computador, setor, usuário ou ID AnyDesk..." className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm font-medium outline-none transition focus:border-[#d6a33a] focus:bg-white focus:ring-4 focus:ring-[#d6a33a]/10" />
        </div>
      </div>
            ...
            return (
              <section key={org} className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_12px_32px_rgba(7,24,45,.05)]">
                <header className={`bg-gradient-to-r ${theme.header} px-5 py-5 text-white sm:px-6`}>
                  <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="grid size-12 place-items-center rounded-2xl bg-white/10"><Building2 size={23}/></div><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-white/60">Órgão</p><h2 className="text-lg font-black">{org}</h2></div></div><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-bold">{items.length} dispositivo{items.length === 1 ? "" : "s"}</span></div>
                </header>
