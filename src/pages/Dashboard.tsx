  ...
  return (
    <div className="space-y-8 lg:space-y-9">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi title="Saldo em caixa" value={cash} icon={<Wallet size={27} />} tone="gold" />
        <Kpi title="Entradas do mês" value={ins} icon={<ArrowUpRight size={27} />} tone="green" />
        <Kpi title="Saídas do mês" value={outs} icon={<ArrowDownRight size={27} />} tone="red" />
        <Kpi title="Valor em conta" value={account} icon={<Landmark size={27} />} tone="blue" />
      </div>
        ...
        {rows.length ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-[15px]">
              <thead>
                <tr>{["Data", "Descrição", "Categoria", "Tipo", "Valor", "Status", "Ações"].map((x) => <th key={x} className="px-6 py-4">{x}</th>)}</tr>
              </thead>
              <tbody>
                {rows.map((x) => (
                  <tr key={x.id}>
...
function PanelHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="border-b border-[#173f68] bg-[#10365f] px-6 py-4 text-white sm:px-7">
      <h3 className="text-[20px] font-extrabold">{title}</h3>
      <p className="mt-1 text-[13px] font-medium text-slate-200">{subtitle}</p>
    </div>
  );
}

function Kpi({ title, value, icon, tone }: { title: string; value: number; icon: any; tone: "gold" | "green" | "red" | "blue" }) {
  const tones = {
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#cad5e3] bg-white shadow-[0_8px_20px_rgba(15,35,70,.08)]">
      <div className={`h-3 bg-gradient-to-r ${tones[0]}`} />
      <div className="flex min-h-[172px] items-start justify-between gap-4 p-6">
        <div className="min-w-0">
          <p className="text-[15px] font-extrabold uppercase tracking-[.035em] text-slate-600">{title}</p>
          <b className="mt-4 block break-words text-[32px] font-extrabold leading-tight tracking-[-.035em] text-[#091a35] sm:text-[35px]">{money(value)}</b>
          <p className="mt-3 text-[12px] font-medium text-slate-400">Atualizado com os lançamentos registrados</p>
        </div>
        <span className={`grid size-14 shrink-0 place-items-center rounded-md ${tones[1]}`}>{icon}</span>
      </div>
    </div>
  );
}
...
function Status({ label, value, count, color }: { label: string; value: number; count: number; color: string }) {
  ...
  return (
    <div className="flex min-h-[88px] items-center justify-between gap-4 rounded-md border border-[#d7e0ea] bg-[#f7f9fc] p-4.5">
      <div className="flex items-center gap-3.5">
        <i className={`size-3.5 rounded-full ${c}`} />
        <div>
          <b className="block text-[16px] font-extrabold text-slate-800">{label}</b>
          <span className="mt-1 block text-[13px] font-medium text-slate-500">{count} contas</span>
        </div>
      </div>
      <strong className="whitespace-nowrap text-[18px] font-extrabold text-slate-800">{money(value)}</strong>
    </div>
  );
}
