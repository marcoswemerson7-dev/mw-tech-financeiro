    ...
    <div className="space-y-5 pb-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_8px_30px_rgba(15,35,70,.05)]">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <button onClick={load} disabled={loading} className="inline-flex min-h-[42px] items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-black text-[#071d35] transition hover:bg-slate-50 disabled:opacity-60">
        ...
        <>
          <section className="grid gap-4 lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              ))}</div> : <div className="grid min-h-[180px] place-items-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-center text-sm text-slate-500"><div><FolderOpen className="mx-auto mb-2 text-slate-400"/><b>Nenhuma pasta configurada</b><p className="mt-1">Cadastre as pastas em GOOGLE_DRIVE_FOLDERS na função.</p></div></div>}
            </div>
          </section>
        </>
      ) : null}
