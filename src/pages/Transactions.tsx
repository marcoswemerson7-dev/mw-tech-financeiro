...
export default function Transactions() {
  ...
  async function remove(movement: Movement) {
    ...
    const message = linked
      ? `Este lançamento foi gerado por uma despesa/pagamento.\n\nExcluir agora removerá também os registros vinculados dessa despesa quando permitido e ajustará o saldo pelo efeito líquido.\n\nDeseja continuar?`
      : `Excluir esta movimentação de ${money(movement.valor)}? O efeito no saldo será estornado automaticamente.`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm xl:grid-cols-[1fr_185px_185px_190px]">
        <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={19} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar descrição, prefeitura, fornecedor..." className="h-13 w-full rounded-lg border border-slate-200 pl-11 pr-4 outline-none focus:border-blue-400" /></div>
        <label className="relative"><CalendarRange className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="h-13 w-full rounded-lg border border-slate-200 pl-10 pr-2" title="Data inicial" /></label>
        <label className="relative"><CalendarRange className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="h-13 w-full rounded-lg border border-slate-200 pl-10 pr-2" title="Data final" /></label>
