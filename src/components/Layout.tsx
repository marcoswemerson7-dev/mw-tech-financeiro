...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#f2f5f9] text-slate-900">
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-[278px] flex-col bg-gradient-to-b from-[#061426] via-[#07182d] to-[#061426] text-white shadow-2xl shadow-slate-950/20 transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="relative flex h-[154px] items-center justify-center border-b border-white/10 px-6">
          <img
            src="/mw-tech-logo.png"
            className="h-[124px] w-[190px] object-contain"
        ...
        <p className="px-6 pb-3 pt-7 text-[11px] font-bold uppercase tracking-[.22em] text-slate-400">
          Menu principal
        </p>
        ...
        <nav className="flex-1 space-y-2.5 px-4">
          {items.map(([to, label, I]) => (
            <NavLink
              ...
              onClick={() => setOpen(false)}
              className={({ isActive }) =>
                `flex min-h-[56px] items-center gap-4 rounded-xl border-l-[4px] px-4 py-3 text-[15px] font-bold transition-all ${isActive ? "border-l-[#e8ac35] bg-white/[.075] text-[#f5c75b] shadow-[inset_0_0_0_1px_rgba(255,255,255,.04)]" : "border-l-transparent text-slate-200 hover:bg-white/[.055] hover:text-white"}`
              }
            >
        ...
        <div className="border-t border-white/10 p-4">
          <button
            onClick={logout}
            className="flex min-h-[50px] w-full items-center gap-3 rounded-xl px-4 py-3 text-[14px] font-semibold text-slate-300 transition hover:bg-white/[.06] hover:text-white"
          >
            <LogOut size={19} />
            Sair
          </button>
          <div className="mt-3 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.06] p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,.08)]">
