  ...
  return <div className="space-y-7">
    ...
    {rows.length ? <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-[14px]">
        ...
        <tbody>{rows.map((item)=><tr key={item.id} className="border-t border-slate-100">
          <td className="px-6 py-5"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-700"><UserRound size={20}/></span><div><b className="block text-[#061426]">{item.nome}</b><span className="text-[12px] text-slate-500">{item.email}</span>{item.cpf && <span className="mt-0.5 block text-[11px] font-semibold text-slate-400">CPF: {item.cpf}</span>}</div></div></td>
    ...
    {edit && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4"><div className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-2xl bg-white">
      ...
      <form onSubmit={save} className="grid gap-5 p-7 sm:grid-cols-2">
        ...
        <label className="text-sm font-bold text-slate-600">E-mail<input name="email" type="email" required defaultValue={edit.email||""} className="input" placeholder="usuario@empresa.com"/></label>
        <label className="text-sm font-bold text-slate-600">CPF<input name="cpf" inputMode="numeric" defaultValue={edit.cpf||""} className="input" placeholder="000.000.000-00"/></label>
        ...
        <p className="sm:col-span-2 rounded-xl border border-blue-100 bg-blue-50 p-4 text-[13px] leading-6 text-blue-900">Informe o mesmo e-mail da conta existente no Appwrite Auth. O CPF ficará vinculado a essa conta e poderá ser usado no login junto com o e-mail.</p>
